...
namespace sdds {
	...
	TennisLog::TennisLog(const TennisLog& src)
	{
		...
		this->matches = new TennisMatch[count];
		for (size_t i = 0; i < count; i++) {
			this->matches[i] = src.matches[i];
		}
	}
	...
	TennisLog::TennisLog(TennisLog&& src) : count(src.count), matches(src.matches)
	{
		//std::move(src);
		src.count = 0;
		src.matches = nullptr;
